...
int get_student(int fd, int id, student_t *s){
    ...
    if (is_file_location_empty(s, STUDENT_RECORD_SIZE)){
        return SRCH_NOT_FOUND;
    }
    return NO_ERROR;
}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    if (! is_file_location_empty(&student, STUDENT_RECORD_SIZE)) {
        if (student.id == id){
...
int del_student(int fd, int id){
    student_t student = EMPTY_STUDENT_RECORD;
    int student_found = get_student(fd, id, &student);
    if (student_found != 0){
...
int count_db_records(int fd){
    ...
    lseek(fd, 0, SEEK_SET);
    student_t student = EMPTY_STUDENT_RECORD;
    while( read(fd, &student, STUDENT_RECORD_SIZE)){
        if( ! is_file_location_empty(&student, STUDENT_RECORD_SIZE)){
            record_count++;
        }
    }
...
/*  
 *   This function checks if the given buffer is filled with 0's
 * 
 *  returns:    int       0 on success, indicating file location is empty
 * 
 *  console:  This function does not produce any output
*/
int is_file_location_empty(void *buffer, int size){
    char arr_zero[size];
    
    for(int i=0; i<size; i++){
        arr_zero[i] = 0;
    }
    return memcmp(buffer, arr_zero, size) == 0;
}
