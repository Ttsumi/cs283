...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        //  - etc.
        if (rsh_match_command(cmd_list.commands[0].argv[0]) == BI_NOT_BI) {
            ...
            if (cmd_rc == BI_CMD_EXIT || cmd_rc == BI_CMD_STOP_SVR) {
                send_message_eof(cli_socket);
                free_cmd_list(&cmd_list);
                free(io_buff);
                return (cmd_rc == BI_CMD_EXIT) ? OK : OK_EXIT;
                return OK;
            } else if (cmd_rc == BI_CMD_RC){
                char last_rc_str[3];
                snprintf(last_rc_str, sizeof(last_rc_str), "%d\n", last_rc);
